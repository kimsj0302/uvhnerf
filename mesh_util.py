...
class mesh_util:
    def __init__(self,obj_path,disp,disp_up,disp_down,data_name="blender",voxel_size=0.02,deform_mesh=""):
        ...
        if os.path.exists(deform_mesh):
            _v, _, _, _f, _ , _ = igl.read_obj(deform_mesh)
            df_mask = np.linalg.norm(v-_v,axis=-1)>1e-5
            v[df_mask] = _v[df_mask]
            vn = mesh2normal(_v,_f)
        ...
        self.face_inv_idx = torch.arange(f.shape[0])[...,None].repeat(1,3).reshape(-1).to(device_mesh)
    ...
    def convert(self,pts,vds):
        ...
        uvh = self.remaper.map(uvh_initial)
