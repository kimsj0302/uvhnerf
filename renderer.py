...
def OctreeRender_trilinear_fast(rays, tensorf, mesh, chunk=4096, N_samples=-1, ndc_ray=False, white_bg=True, is_train=False, step_size=0.001, h_ratio = 1.0, device='cuda:0'):
    ...
    for chunk_idx in range(N_rays_all // chunk + int(N_rays_all % chunk > 0)):
        ...
        with torch.no_grad():
            ...
            _uvh, valids, newdirs, _ = mesh.convert(xyz_sampled[ray_valid],viewdirs[ray_valid])
...
@torch.no_grad()
def evaluation(test_dataset,tensorf, mesh,args, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
    ...
    for idx, samples in tqdm(enumerate(test_dataset.all_rays[0::img_eval_interval]), file=sys.stdout):
        ...
        rgb_map, _, depth_map, _, _ = renderer(rays, tensorf, mesh,chunk=4096, N_samples=N_samples,
