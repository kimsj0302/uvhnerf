...
class mesh_util:
    def __init__(self,obj_path,uv,uv_mask,disp,disp_up,disp_down,data_name="blender",voxel_size=0.02):
        
        # v, tc, vn, f, _ , fn = igl.read_obj(obj_path)
        ...
        # tc_edge_np = dup_edge(remap_f_fn,f,tc)
        # self.tc_edge = torch.Tensor(tc_edge_np).to(device_mesh)
        ...
        # np.ceil((v.max(0)+disp)/voxel_size) - np.ceil((v.min(0)-disp)/voxel_size)
        path_split = os.path.split(obj_path)
        ...
        # breakpoint()
        # __u = np.array(new_v)+np.array(max_disp[:,None]*new_vn)
        # __d = np.array(new_v)+np.array(min_disp[:,None]*new_vn)
        # igl.write_triangle_mesh("test_up.obj",__u,new_f)
        # igl.write_triangle_mesh("test_down.obj",__d,new_f)
        
        self.setup_grid()
    
        # self.xyz2mesh = torch.Tensor([[-1,0,0],[0,0,-1],[0,1,0]]).to(torch.float32).cuda()
        # y->x , z->y, x->z
        # xyz -> zxy
        # yzx <- xyz
        ...
        # ## 13736 , 13732
        # breakpoint()
        # save_tet_ids(range(0,))
        # self.save_tet_ids(range(30000,40000))
    ...
    def convert(self,pts,vds):
        ...
        # out,inv = scatter_min(ini_cost,pts_id_masked)
        # mask_bary = out < (1.0 + 1e-6)
        ...
        # tight_out_mask,_ = scatter_max(1.0*tight_val,pts_id_masked)
        # tight_out,tight_inv = scatter_max(10.0-torch.abs(0.5-torch.abs(1.-ini_sol.sum(1))) * tight_val,pts_id_masked)
        
        
        # tight_val,tight_inv = scatter_max(1.0-torch.abs(ini_sol.sum(1) - 0.5) * tight_val,pts_id_masked)
        ...
        result_mask = torch.nn.functional.grid_sample(self.uv_mask[None,None,...],grid[None,None,...],align_corners=True)
        new_uv = result[0,:,0,:].permute(1,0)
        new_uv_mask = result_mask[0,0,0,:]
        uvh_target = torch.cat((new_uv,h_initial),-1).to(uvh_initial.device)
        if new_uv_mask.max()>0.0:
            ...
            uvh_target[new_uv_mask>0.0] = torch.cat((new_uv_nn,h_initial),-1).to(uvh_initial.device)[new_uv_mask>0.0]
            # breakpoint()
        ...
        return uvh.to(device_model), valids.to(device_model), vds[valids].to(device_model), _uvh_initial.to(device_model)
